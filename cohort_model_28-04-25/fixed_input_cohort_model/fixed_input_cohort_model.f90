...
!******************************************************************************************************************************
		...
		write(unit,'(a)') "from concurrent.futures import ProcessPoolExecutor"
		write(unit,'(a)') ""
		write(unit,'(a)') "files = ["
		...
		write(unit,'(a)') "    fname, title = args"
		write(unit,'(a)') "    data = np.loadtxt(fname, delimiter=',')"
		write(unit,'(a)') "    x, y, vals = data[:,0], data[:,1], data[:,2]"
		write(unit,'(a)') "    sf = 1000.0"
		write(unit,'(a)') "    rows = 1 + int((y.max()-y.min())/sf)"
		write(unit,'(a)') "    cols = 1 + int((x.max()-x.min())/sf)"
		write(unit,'(a)') "    grid = np.full((rows, cols), np.nan)"
		write(unit,'(a)') "    yi = ((y - y.min())/sf).astype(int)"
		write(unit,'(a)') "    xi = ((x - x.min())/sf).astype(int)"
		write(unit,'(a)') "    np.clip(yi, 0, rows-1, out=yi)"
		write(unit,'(a)') "    np.clip(xi, 0, cols-1, out=xi)"
		...
		write(unit,'(a)') "    fig.savefig(out, dpi=500)"
		write(unit,'(a)') "    plt.close(fig)"
		write(unit,'(a)') "    return out"
		write(unit,'(a)') ""
		write(unit,'(a)') "if __name__ == '__main__':"
		write(unit,'(a)') "    with ProcessPoolExecutor() as executor:"
		write(unit,'(a)') "        for output_path in executor.map(plot_one, files):"
		write(unit,'(a)') "            print(f'Wrote {output_path}')"
